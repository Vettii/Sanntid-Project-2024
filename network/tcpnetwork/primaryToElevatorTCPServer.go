...
// PrimaryToElevatorTCPServer is a module that handles the TCP communication between the primary and the elevators
// Interface consists of the following:

// -------------------------------------
// - NewPrimaryToElevatorTCPServer: A function that initializes a new PrimaryToElevatorTCPServer
// - Out: A channel for sending data to the elevators, must prefix every message with the elevator ID to send to the correct elevator
// - In: A channel where data from the elevators is received
// - Run: A function that runs the module
// - Stop: A function that stops the module and closes all connections
// - GetActiveConnections: A function that returns a list of active connections
// -------------------------------------
...
func (server *PrimaryToElevatorTCPServer) checkIfAlive() { //Maybe this isn't so good. Not very acceptance test oriented. Maybe make it so that every socket resets a timer on a read or write, if that timer runs that means that all socket are timed out and dead
	for {
		select {
		case <-server.stop:
			server.active = false
...
func (server *PrimaryToElevatorTCPServer) deleteSingleConnection(ID int) { //Will panic if connection is already closed. Maybe implement a check for that
